...
const About: React.FC = () => {
  const { images, sections, sectionRotationInterval, imageRotationInterval } = DATA.about;
  const [currentIndex, setCurrentIndex] = useState(0);
  const [currentSection, setCurrentSection] = useState(0);
  const [isPaused, setIsPaused] = useState(false);
  const intervalId = useRef<NodeJS.Timeout | null>(null);
  const sectionIntervalId = useRef<NodeJS.Timeout | null>(null);
  ...
  }, [currentIndex, images.length, imageRotationInterval]);

  useEffect(() => {
    if (!isPaused) {
      sectionIntervalId.current = setInterval(() => {
        setCurrentSection((prev) => (prev + 1) % sections.length);
      }, sectionRotationInterval);
    }
  ...
  }, [isPaused, sections.length, sectionRotationInterval]);
    ...
    <section className="about section" id="about">
      ...
      <div className="about_container container grid">
        ...
        <div className="about_data">
          ...
          <a href={DATA.about.ctaLink} className="button button--flex">
            {DATA.about.ctaText}
            <svg
