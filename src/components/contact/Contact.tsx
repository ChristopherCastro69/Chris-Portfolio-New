  ...
  const sendEmail = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus({
          type: 'error',
          message: data.error || 'Failed to send message. Please try again or contact me directly via email.',
        });
      }
    } catch {
      setStatus({
    ...
    <section className="contact section" id="contact">
      ...
      <div className="contact_container container grid">
        <div className="contact_content">
          <h3 className="contact_title">{DATA.contact.sectionTitle}</h3>

          <div className="contact_info">
            {DATA.contact.cards.map((card) => (
              <div className="contact_card" key={card.id}>
                <i className={`bx ${card.icon} contact_card-icon`}></i>
                <h3 className="contact_card-title">{card.title}</h3>
                <span className="contact_card-data">{card.data}</span>
                <a
                  href={card.link}
                ...
                >
                  {card.linkText} <i className="bx bx-right-arrow-alt contact_button-icon"></i>
                </a>
              </div>
            ))}
          </div>
        ...
        <div className="contact_content">
          <h3 className="contact_title">{DATA.contact.formTitle}</h3>

          {status.message && (
            <div className={`contact_status contact_status--${status.type}`}>
              {status.message}
            </div>
          )}
          ...
          <form ref={form} onSubmit={sendEmail} className="contact_form">
            ...
            <div className="contact_form-div contact_form-area">
              <label className="contact_form-tag">Question</label>
              <textarea
                name="question"
                cols={30}
                rows={10}
                className="contact_form-input"
                placeholder={DATA.contact.form.questionPlaceholder}
                required
                disabled={isSubmitting}
              ></textarea>
            </div>
            ...
            <button className="button button--flex" type="submit" disabled={isSubmitting}>
              {isSubmitting ? 'Sending...' : DATA.contact.form.submitText}
